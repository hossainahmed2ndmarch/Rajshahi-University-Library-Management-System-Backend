...
exports.sendMembershipNoticeAlert = exports.sendShiftCancellationAlert = exports.sendShiftScheduleAlert = exports.sendOverdueAlert = void 0;
const emailSender_1 = __importDefault(require("./emailSender"));
const sendOverdueAlert = (_a) => __awaiter(void 0, [_a], void 0, function* ({ userName, userEmail, userPhone, bookTitle, dueDate, borrowId, }) {
  ...
  `;
    ...
    console.log(`[SMS / Mobile Overdue Alert] Sent to ${userPhone} (${userName}): "Assalamu Alaikum ${userName}, the book '${bookTitle}' borrowed from RU Islamic Library is OVERDUE (Due: ${formattedDueDate}). Please return it to the library counter as soon as possible."`);
});
exports.sendOverdueAlert = sendOverdueAlert;
const sendShiftScheduleAlert = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    const { shifterName, shiftStartTime, shiftEndTime, shiftSlotName, recipients, notificationMethod = 'EMAIL', notes, } = payload;
    ...
    for (const recipient of recipients) {
      ...
      `;
        ...
        if (notificationMethod === 'SMS' || notificationMethod === 'ALL') {
            console.log(`[SMS / Mobile Shift Schedule Alert] Sent to ${recipient.phone || recipient.email} (${recipient.name}): "RUIL: Shifter ${shifterName} scheduled duty shift on ${startFormatted} - ${endFormatted} (${shiftSlotName || 'Duty Slot'})."`);
        }
    }
});
exports.sendShiftScheduleAlert = sendShiftScheduleAlert;
const sendShiftCancellationAlert = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    const { shifterName, shiftStartTime, shiftEndTime, shiftSlotName, recipients, notificationMethod = 'EMAIL', reason, } = payload;
    const startFormatted = new Date(shiftStartTime).toLocaleString('en-US', {
        weekday: 'short',
        month: 'short',
        day: 'numeric',
    ...
    for (const recipient of recipients) {
      ...
      `;
        ...
        if (notificationMethod === 'SMS' || notificationMethod === 'ALL') {
            console.log(`[SMS / Mobile Shift Cancellation Alert] Sent to ${recipient.phone || recipient.email} (${recipient.name}): "RUIL ALERT: Shifter ${shifterName} CANCELLED duty on ${startFormatted}. Reason: ${reason || 'Advance notice'}. Please arrange replacement."`);
        }
    }
});
exports.sendShiftCancellationAlert = sendShiftCancellationAlert;
const sendMembershipNoticeAlert = (payload) => __awaiter(void 0, void 0, void 0, function* () {
...
exports.sendMembershipNoticeAlert = sendMembershipNoticeAlert;
